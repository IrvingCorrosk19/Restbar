    ...
    public class AdvancedSettingsController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            try
            {
                ...
                var printers = await _printerService.GetAllAsync(companyId);
        ...
        public async Task<IActionResult> Printers()
        {
            ...
            {
                var user = await GetCurrentUserAsync();
                var printers = await _printerService.GetAllAsync(user?.Branch?.CompanyId);
                return View(printers);
            }
            catch (Exception ex)
            {
                TempData["Error"] = $"Error al cargar las impresoras: {ex.Message}";
        ...
        public async Task<IActionResult> CreatePrinter(Printer printer)
        {
            try
            {
                ...
                var user = await GetCurrentUserAsync();
                printer.CompanyId = user?.Branch?.CompanyId;

                var createdPrinter = await _printerService.CreateAsync(printer);
                TempData["Success"] = $"Impresora {createdPrinter.Name} creada exitosamente";
                return RedirectToAction(nameof(Printers));
            }
            catch (Exception ex)
            {
                TempData["Error"] = $"Error al crear la impresora: {ex.Message}";
                return View(printer);
            }
        }
