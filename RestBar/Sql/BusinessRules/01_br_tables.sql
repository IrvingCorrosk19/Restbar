-- RB-029 Business Rules Engine (idempotent)
...
CREATE TABLE IF NOT EXISTS br_rule_versions (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    rule_id uuid NOT NULL REFERENCES br_rules(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS br_rule_conditions (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    rule_version_id uuid NOT NULL REFERENCES br_rule_versions(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS br_rule_execution_logs (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    execution_id uuid NOT NULL REFERENCES br_rule_executions(id) ON DELETE CASCADE,
...
VALUES ('20260731050000_BusinessRulesEnterprise', '9.0.5')
