            ...
            try
            {
                ...
                var metrics = await query
                    ...
                    .Select(g => new SalesMetrics
                    {
                        ...
                        TotalDiscounts = g.SelectMany(o => o.OrderItems).Sum(oi => oi.Discount ?? 0),
                        NetRevenue = g.Sum(o => o.TotalAmount ?? 0) - g.SelectMany(o => o.OrderItems).Sum(oi => oi.Discount ?? 0),
            ...
            try
            {
                ...
                var dailySales = await query
                    ...
                    .Select(g => new DailySalesData
                    {
                        ...
                        AverageTicket = g.Average(o => o.TotalAmount ?? 0),
                        Discounts = g.SelectMany(o => o.OrderItems).Sum(oi => oi.Discount ?? 0)
                    })
                    .OrderBy(d => d.Date)
                    .ToListAsync();
            ...
            try
            {
                ...
                var discounts = await query
                    ...
                    .Select(oi => new DiscountData
                    {
                        ...
                        DiscountedPrice = oi.UnitPrice - (oi.Discount ?? 0),
                        DiscountAmount = oi.Discount ?? 0,
                        DiscountPercentage = oi.UnitPrice > 0 ? ((oi.Discount ?? 0) / oi.UnitPrice) * 100 : 0,
                        OrderDate = oi.Order.ClosedAt ?? DateTime.Now,
                        EmployeeName = oi.Order.User.FullName ?? "Sin nombre",
                        BranchName = "Sucursal Principal", // Por ahora hardcodeado
                        Quantity = (int)oi.Quantity,
                        TotalDiscount = (oi.Discount ?? 0) * oi.Quantity
                    })
